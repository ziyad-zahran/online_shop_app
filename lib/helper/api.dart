...
class Api {
  Future<List<dynamic>> get({required String url,@required token}) async
  {
  ...
  Future<dynamic> post({required String url,@required dynamic body,@required String? token}) async
  {
    ...
    if (response.statusCode == 200) {
      ...
      {
        throw Exception('there is a problem with status code ${response.statusCode} and with body ${response.body}');
      }
  }
